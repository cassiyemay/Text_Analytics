...
def evaluate_features(feature_select):
    ...
    negSentences = open(os.path.join(__location__, 'rt-polarity-neg-short.txt'), 'r', encoding='utf8')
    posSentences = open(os.path.join(__location__, 'rt-polarity-pos-short.txt'), 'r', encoding='utf8')
    negSentences = re.split(r'\n', negSentences.read())
    posSentences = re.split(r'\n', posSentences.read())
    posFeatures = []
    negFeatures = []
    stop1 = open(os.path.join(__location__, 'stopwords.txt'), 'r')
    stop = re.split(r'\n', stop1.read())
    print(stop)
    ...
    for i in posSentences:
        ...
        for word in posWords:
            if word not in stop:
                aa.append(word)
        posWords = [feature_select(aa), 'pos']
    ...
    for i in negSentences:
            ...
            for words in negWords:
                if words not in stop:
                    bb.append(words)
            negWords = [feature_select(bb), 'neg']
